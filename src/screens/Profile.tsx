...
const Profile = () => {
  const { username } = useParams<ProfileParam>();
  const { data: userData } = useUser();
  const client = useApolloClient();
  const unfollowUserUpdate = (cache: ApolloCache<object>, result: any) => {
    const {
      data: {
        unfollowUser: { ok },
      },
  ...
  const { data, loading } = useQuery<SeeProfileProps>(SEE_PROFILE_QUERY, {
    variables: {
      username,
    },
  });
